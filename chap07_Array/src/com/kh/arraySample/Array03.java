...
public class Array03 {
	...
	public static void main(String[] args) {
		...
		//String 안에 char배열로 만들어준다 ->toCharArrey
		//ex) String 안녕하세요 = {'안' '녕' '하' '세' '요'}
		System.out.println("검색할 문자를 입력하세요.");//검색할 문자
		char searchChar = sc.next().charAt(0); //첫번쨰
		...
		System.out.println("문자열 개수 체크 : " +charCount);
		
		
		//문자열에 몇 개 들어가 있는지 개수 -> length
		//몇 번째 인덱스에 위치하는지 인덱스 출력하기 -> 인덱스 출력
		//총 길이가 얼마나 되는지 확인하고, 각 자리값 출력하기
		int count = 0;//검색할 문자 개수 카운트
		System.out.println(inputString + " 에 " + searchChar + "가 존재하는 위치(인덱스)");
		//반복문을 사용해서 위치 찾기
		for (int i = 0; i < charCount; i++) {
		//만약에 내가 이 문자를 검색했을때 이 문자는몇 번째 인덱스에 위치하는지 인덱스 출력하기
			if (charString[i] == searchChar) {
				//charString[i] 고정값이 아니라 i값이 되어야 하는 이유는
				//searchChar 우리가 검색해서 문자값을 지정해주기 때문에 값이 고정되어 잇어야함
